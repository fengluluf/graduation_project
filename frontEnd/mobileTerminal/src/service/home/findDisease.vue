                    ...
                    <van-tabs color="#28b8a1" ellipsis animated @click="onClick" sticky scroll="{scrollTop:146px,isFixed:true}">
                        <van-tab v-for="(item,key) in sortList" :title="item.name" :key="key">
                            <van-collapse v-model="activeNames1">
                                <van-collapse-item v-for="(item1,key1) in diseaseList" :key="key1" :title="item1.title" :name="item1.name" v-if="item1.child.child">
                                    <div v-for="(item2,key2) in item1.child" :key="key2" class="disDiv" @click="goDiseaseDetail(item2)">{{item2.title}}</div>
                                </van-collapse-item>

                                <van-collapse-item v-for="(item1,key1) in diseaseList" :key="key1" :title="item1.title" :name="item1.name" v-else>
...
export default {
    ...
    data() {
        return {
            ...
            diseaseList:[{id:1,title:'呼吸系统疾病',name:'1',
            child:[{id:101,title:'急性上呼吸道感染及急性气管炎',name:'101',
            child:[{id:10101,title:'急性气管-支气管炎',name:'10101'},
